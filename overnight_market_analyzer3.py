...
def fetch_market_data(ticker: str, start_date, end_date) -> pd.DataFrame:
    """
    Fetch historical OHLC data from Yahoo Finance for a single ticker.
...
def fetch_all_market_data(
    ...
    """
    Fetch data for every market in parallel using a bounded thread pool.

    Sequential fetching of 140 tickers costs 140 x network_latency.
    Parallel fetching reduces that to roughly max(network_latency).

    Args:
        tickers:    {market_name: ticker_symbol} mapping.
        start_date: Passed through to fetch_market_data.
        end_date:   Passed through to fetch_market_data.

    Returns:
        {market_name: DataFrame} — empty DataFrame for any failed fetch.
    """
    market_data: Dict[str, pd.DataFrame] = {}

    with ThreadPoolExecutor(max_workers=MAX_FETCH_WORKERS) as executor:
        future_to_name = {
            executor.submit(fetch_market_data, ticker, start_date, end_date): name
            for name, ticker in tickers.items()
        }
        for future in as_completed(future_to_name):
            name = future_to_name[future]
            try:
                market_data[name] = future.result()
            except Exception:
